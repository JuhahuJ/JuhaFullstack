...
describe('total likes', () => {
    ...
    test('when list has only one blog with no likes equals the likes of that', () => {
        const result = listHelper.totalLikes(listWithOneBlogNoLikes)
        assert.strictEqual(result, 0)
    })
    test('when list has multiple blogs with no likes equals the likes of those', () => {
        const result = listHelper.totalLikes(blogsNoLikes)
        assert.strictEqual(result, 0)
    })
})
